...
public class GraphUtils {
  ...
  public static OptimizedMcfGraph buildOptimizedMcfGraph(List<McfGraph> graph) {
    List<McfStatVarObsSeries> svo_list = new ArrayList<>();
    for (McfGraph g : graph) {
      for (PropertyValues pv : g.getNodesMap().values()) {
        if (isObservation(pv)) {
          ...
          svo_list.add(svoSeries);
        }
      }
    }

    Map<McfStatVarObsSeries.Key, List<McfStatVarObsSeries>> svo_by_key =
        svo_list.stream().collect(Collectors.groupingBy(McfStatVarObsSeries::getKey));
    OptimizedMcfGraph.Builder res = OptimizedMcfGraph.newBuilder();
    svo_by_key.forEach(
        (K, V) -> {
          McfOptimizedGraph.Builder g = McfOptimizedGraph.newBuilder();

          McfStatVarObsSeries.Builder sv_obs_series = McfStatVarObsSeries.newBuilder();
          sv_obs_series.setKey(K);
          for (McfStatVarObsSeries svo : V) {
            sv_obs_series.addAllSvObsList(svo.getSvObsListList());
          }
          g.setSvObsSeries(sv_obs_series.build());
